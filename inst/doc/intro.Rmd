---
title: "Introduction to StatComp22081"
author: "22081"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to StatComp22081}
...
__StatComp22081__ is a simple R package developed to compare the performance of a self coded function _regres_ ,which could preform standard regression,ridge regression and locally weighted linear regression (using Gaussian kernel) for a given data frame(the label variable has to stay at the last columns of the data frame,like 'Boston' in MASS),we compare the predicted value of label variable by the three means mentioned above with that calculated by function lm() using Cross-Validation method.
This package also contains a cpp function _targetoutofn_  to calculate the  probability of  a certain number 'target' out of n (target <= n) delivery man with respectively different probability to accept an order.



## CrossValidation  _reg.R_ and _lm_

The source R code for _reg.R_ is as follows:
...
The above code could use functions in R which are _lm_to achieve as follows:

```{r,eval=FALSE}
summary(standard <- lm(y ~ ., data = longley))
```

Cross-Validation for standard regression and ridge regression:
...
The above result shows that self coded standard regression comes out excatly the same result as function lm(),and self coded ridge regression preform not so good as standard regression do,which is probably because features in the data we used are not so correlated with each other. 

## Cross-validation of the thrid method,i.e. locally weighted linear regression.
...
From the result we could see that the variance is much smaller then standard regression and ridge regression.
